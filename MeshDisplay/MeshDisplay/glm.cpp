...
glmVertexNormals(GLMmodel* model, GLfloat angle)
{
    ...
    model->initNormals = (GLfloat*)malloc(sizeof(GLfloat)* 3 * (model->numnormals + 1));
...
glmReadOBJ(char* filename)
{
    ...
    model->initVertices = (GLfloat*)malloc(sizeof(GLfloat)*
        3 * (model->numvertices + 1));
...
glmDraw(GLMmodel* model, GLuint mode)
{
    ...
    while (group) {
        ...
        for (i = 0; i < group->numtriangles; i++) {
            ...
            //// Calculate new normal based on transformed vertices
            //Vec3 v0, v1, v2;
